...
prepare_next_base_version() {
    ...
    if [[ -z "$Latest_Alpha_Branch_Base_Version" ]]; then
        ...
        next_base_version="${version_parts[0]}.$new_minor.1"
    fi
...
Current_Dev_Channel_Number=$(determine_channel_number $CHANNEL_NAME $Next_Dev_Channel_Base_Version)
Current_Dev_Channel_Version="${Current_Dev_Channel_Base_Version}-${CHANNEL_NAME}.${Current_Dev_Channel_Number}"
...
case "$VERSION_TYPE" in
    latest)
        SELECTED_VERSION="$Current_Dev_Channel_Version"
        ;;
    current)
        SELECTED_VERSION="$Current_Dev_Channel_Version"
        ;;
...
if $DEBUG; then
    ...
    echo "CDCV: $Current_Dev_Channel_Version"
