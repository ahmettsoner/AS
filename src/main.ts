...
program
  ...
  programPhase.addCommand(
      ...
      .action(async (channel: string, version: string, commandOptions: any) => {
          ...
          const branchManager = new BranchManager() //burdan devam
          const determinedVersion = await branchManager.Determ ineTestPhaseVersion(channel, version, options);
          if(determinedVersion){
            console.log(determinedVersion.replace("\n", ""))
          }
      })
  ...
  programTag.addCommand(
    new Command()
      ...
      .addOption(
        new Option('-p, --print <type>', 'Print option can be full, base, channel, or left empty')
