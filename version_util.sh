...
# Function to find the latest version from release branches
get_latest_release_branch_version() {
    local LATEST_VERSION=""
    local BRANCHES

    # Fetch only local branches that match the release pattern
    BRANCHES=$(git branch --list 'release/v[0-9]*.[0-9]*.0-alpha')

    while IFS= read -r branch; do
        # Strip the prefix and parse the version numbers
        if [[ "$branch" =~ release/v([0-9]+)\.([0-9]+)\.[0-9]+-alpha ]]; then
